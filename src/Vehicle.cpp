...
void VehicleSimulator::simulate_braking(double duration, double dt) {
  states.clear();
  double t = 0.0, pos = 0.0, vel = 27.78;  // 100 km/h
  const double g = 9.81;
...
void VehicleSimulator::simulate_cruise(double target_speed_ms, double duration, double dt) {
  ...
  while (t <= duration) {
    ...
    double engine_force = speed_error * 1000.0;
    if (engine_force < 0.0) engine_force = 0.0;
    if (engine_force > max_engine_force) engine_force = max_engine_force;
...
void VehicleSimulator::simulate_mixed_profile(double total_time, double dt) {
  (void)total_time;  // ← FIXES THE WARNING

  states.clear();
  double t = 0.0, pos = 0.0, vel = 0.0;
  const double g = 9.81;

  // Phase 1: Accelerate (0-5s)
  while (t < 5.0) {
    double engine_force = max_engine_force;
    double drag = 0.5 * drag_coeff * vel * vel;
    double friction = rolling_resist * mass * g;
...
void VehicleSimulator::save_to_csv(const std::string& filename) const {
  ...
  for (const auto& state : states) {
    file << state.time << "," << state.position << "," << state.velocity << ","
         << state.acceleration << "," << state.engine_force << "," << state.drag_force << ","
